...
|----|---------|-----|------|--------|-------------|----------|
| FE-1 | Family Freedom Engine / Boring Report | 1 | First sellable proof — paid agent evidence primitive | doing | Use `products/boring-report/scorecard/output/aave-v3-change-log-pilot.md` as the current handoff: compare the 2026-08-16 bundle against the 2026-08-14 baseline, use `products/boring-report/scorecard/output/aave-v3-comparison-result-note-template.md` to capture the literal compare-only result, keep WBTC-first queued for the next bounded asset, and leave publication/payment/account actions gated. | publish/payment/account gate later |
14|| CI-1 | Career/Income | 1 | Global high-compensation cash runway | todo | Anchorage Digital Trading Operations Team Lead is the primary decision packet: live remote-US source remains current on the 2026-08-27 morning sync; packet/brief/source note plus the application-form preflight answer outline are ready. Confirm remote compensation clears $120K (prefer $150K+) and that player-coach leadership scope is acceptable before final tailoring; Ripple is secondary hold only. | **yes** to submit/send |
| WO-2 | Weather Oracle | 2 | Compounding capture loop | blocked | Draft-only cleanup note at `projects/weather-oracle-repo-cleanup-draft.md` is current; the narrow stale-line sweep in the Weather Oracle project docs and queue wording is now done, while keeping the packet / run sheet / result-note bundle aligned. Reopen only after the Boring Report proves paid demand or when a buyer/partner wedge makes Forecast Receipts commercially concrete. | no |
...
| MA-1 | Market Activity | 3 | Market activity tracker | blocked | Draft artifact now lives at `projects/market-activity-config-wiring-draft.md`; keep the lane parked until the CI-1 week-smooth reopen condition is met, then wire `configs/market_watchlist.json` into the fetcher and add daily snapshots. | no |
