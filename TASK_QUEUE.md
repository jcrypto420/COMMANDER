...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft-only: keep the current result-note handoff aligned to the live 200/200 snapshot; next step is the single local-only Pi capture proof, then save the manifest/result note locally. | no |
