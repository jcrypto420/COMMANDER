...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft bundle is complete; run the single local-only Pi capture proof, then save the manifest/result note and verify the `nws_forecast` / `openmeteo_models` gate. | no |
