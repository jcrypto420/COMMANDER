...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | On the Pi, run `products/weather-oracle/capture_daily.py` once and verify a fresh `captures/<timestamp>/manifest.json` with `nws_forecast.json` and `openmeteo_models.json` present and 200 in the manifest. | no |
