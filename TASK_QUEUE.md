...
|----|---------|-----|------|--------|-------------|----------|
|| CI-1 | Career/Income | 1 | Job search per `jobs/SEARCH_PLAYBOOK.md` | doing | 2026-07-14 Coinbase Institutional Business Operations packet is the next decision surface; Josh should approve apply / tweak / kill, while the old IN-1 research-services backstop stays historical | **yes** to send |
||| CC-24 | Command Center | 1 | 5-line daily dispatch | todo | 2026-07-14 07:30 parser re-check passed again; next safe step is 2026-07-15 07:30 parser re-check — assert 5 lines, confirm `Open:` / `Decide:` stay on line 4, keep exactly one bolded `Decision:`, then archive if clean | verify only |
...
||| BB-26 | Bad Boys/Joycat | 1 | Hermes idea-bank + MoA critics cron | todo | 2026-07-24 draft packet now includes a local-only smoke-test checklist tied to `assets/badboys/idea-bank/weekly-template.md`, plus fallback model order and file-shape checks; next safe step is Josh's ship/kill/tweak verdict on cadence + storage path | **yes** |
