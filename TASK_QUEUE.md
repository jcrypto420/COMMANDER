...
|----|---------|-----|------|--------|-------------|----------|
| BR-1 | Boring Report | 1 | Flagship build — P2 Oracle Scorecard v0 | doing | Internal review accepted the verified architecture-evidence baseline (`products/boring-report/scorecard/internal-review-2026-07-30.md`). Next: build one traceable Aave V3 deployment feed-map fact file before reconsidering liveness/concentration or making broader score claims. | internal review gate |
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft-only: 2026-08-09 18:01 live no-login recheck still shows 200/200, so keep the current result-note handoff aligned and run the single local-only Pi capture proof next; then save the manifest/result note locally. | no |
...
| CI-1 | Career/Income | 2 | Opportunistic job pipeline | todo | Coinbase Institutional packet + decision brief are the live decision surfaces; refreshed 2026-08-09 after morning repo sync/rebase/pull recovery; Josh reads the refreshed packet and either approves apply / tweak / kill, then resume the weekly fresh high-fit scan. | **yes** to send |
