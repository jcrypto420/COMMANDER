...
|| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft-only packet/result note now match the live `capture_daily.py` source list; next action is the single Pi one-shot capture verification for `nws_forecast.json` + `openmeteo_models.json` at 200, then save the manifest/result note from the template. | no |
