...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft-only preflight complete: `capture_daily.py` sources and manifest pass condition are confirmed; next run is the Pi one-shot capture verification for `nws_forecast.json` + `openmeteo_models.json` at 200. | no |
