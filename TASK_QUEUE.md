...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft-only: keep the refreshed 2026-08-11 result-note handoff current; the next real step stays the single local-only Pi capture proof, then save the manifest/result note locally. | no |
...
| CI-1 | Career/Income | 2 | Opportunistic job pipeline | blocked | Coinbase Institutional packet + decision brief are parked as a weekly hedge after the 2026-08-11 morning repo sync/rebase/pull recovery; reopen only when Josh explicitly asks for another fresh scan or a send decision. | **yes** to send |
