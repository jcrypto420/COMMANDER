...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft the first-run result note in `projects/weather-oracle-capture-result-note-draft.md`, then keep the lane parked until the single local-only Pi capture proof runs and saves the manifest/result note after verifying the `nws_forecast` / `openmeteo_models` 200 gate. | no |
