...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | The result-note scaffold now carries the literal 2026-08-08 200 preflight snapshot; next step is the single local-only Pi capture proof, then save the manifest/result note after the `nws_forecast` / `openmeteo_models` gate. | no |
