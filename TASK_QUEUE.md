...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft-only: keep the 2026-08-11 handoff aligned; the next real step stays the single local-only Pi capture proof, then save the manifest/result note locally. | no |
...
|| CI-1 | Career/Income | 2 | Opportunistic job pipeline | todo | Coinbase Institutional packet + decision brief are the live decision surfaces; refreshed 2026-08-11 after morning repo sync/rebase/pull recovery; Josh reads the refreshed packet and either approves apply / tweak / kill, then resume the weekly fresh high-fit scan. | **yes** to send |
