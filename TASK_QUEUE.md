...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Use the draft run-sheet failure-modes checklist, then run the single local-only Pi capture proof once and save the manifest/result note after verifying the `nws_forecast` / `openmeteo_models` 200 gate. | no |
