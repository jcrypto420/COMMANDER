...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Draft-only step: draft the exact Pi one-shot capture run sheet for the `nws_forecast.json` + `openmeteo_models.json` 200 gate, including the manifest and result-note checklist; keep the real Pi capture as the next execution step. | no |
