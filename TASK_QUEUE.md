...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | The result-note scaffold is ready; keep the lane parked until the single local-only Pi capture proof runs, then use the scaffold to record the manifest/result note after verifying the `nws_forecast` / `openmeteo_models` 200 gate. | no |
