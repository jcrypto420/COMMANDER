...
|----|---------|-----|------|--------|-------------|----------|
| CI-1 | Career/Income | 1 | Job search per `jobs/SEARCH_PLAYBOOK.md` | doing | 2026-07-11 sync recovery kept the IN-1 draft-only backstop current; next safe step is Josh approval to verify 10 named leads (public URLs, roles, fit notes) and keep all outbound actions draft-only | **yes** to send |
| CC-24 | Command Center | 1 | 5-line daily dispatch | todo | 2026-07-10 parser re-check passed; next safe step is tomorrow's 07:30 parser re-check — assert 5 lines, confirm `Open:` / `Decide:` stay on line 4, keep exactly one bolded `Decision:`, then archive if clean | verify only |
| BB-25 | Bad Boys/Joycat | 1 | CARTOON LAB pilot — P4 "T+2" Gate 2 VERDICT: animation KILLED 2026-07-07 | doing | Josh rejected Blender animation quality ("done trying to animate"); stills/look remain approved. Next: motion-pipeline decision (AI image-to-video research, Josh's go pending) or ship as stills format. Final cut archived at `assets/badboys/cartoon-lab/t2-ep1/t2ep1_master.mp4` | gate 2 |
| BB-26 | Bad Boys/Joycat | 1 | Hermes idea-bank + MoA critics cron | todo | draft packet now also defines the weekly bank file shape + review packet; next safe step is Josh approval to verify the 10 premise prompts and approve the weekly cron surface | **yes** |
