...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | Use `projects/weather-oracle-capture-run-sheet.md` for the next local-only Pi capture proof, then save the result note and verify the manifest gate against the `nws_forecast` / `openmeteo_models` robot-baseline pair. | no |
