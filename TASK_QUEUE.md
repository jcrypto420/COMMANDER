...
|----|---------|-----|------|--------|-------------|----------|
| CI-1 | Career/Income | 1 | Job search per `jobs/SEARCH_PLAYBOOK.md` | doing | Chainlink = ALREADY APPLIED (ChatGPT lane — dedupe caught 2026-07-03). ALL agents check `jobs/TRACKER.md` BEFORE any job work. Next: Coinbase Billing Ops packet → `jobs/packets/` | **yes** to send |
| CC-24 | Command Center | 1 | 5-line daily dispatch | todo | draft the 5-line morning brief target in `projects/command-center-dashboard-v0.md`; tomorrow verify the 07:30 dispatch arrives in 5 lines with ONE bolded decision, then archive | verify only |
...
| CC-22 | Command Center | 2 | Mission Control: lane cards + GATE DECK + PWA | todo | (Josh 2026-07-03: widget-style gates were a hit) parse `## Status` blocks into lane cards; add Gate Deck page — pending decisions as tap-to-verdict cards writing to the inbox API (capture-only, loops read verdicts); add PWA manifest+icons so Josh installs Mission Control as a phone app; pairs with CC-10 Tailscale for anywhere-access | yes to deploy |
