...
| WO-2 | Weather Oracle | 1 | Compounding capture loop | todo | The result-note scaffold and operator note are ready; the live no-login NWS/Open-Meteo inputs were re-checked today (both 200), and the next step is still the single local-only Pi capture proof before recording the manifest/result note after the `nws_forecast` / `openmeteo_models` gate. | no |
...
| CI-1 | Career/Income | 2 | Opportunistic job pipeline | todo | Coinbase Institutional packet + decision brief are the live decision surfaces; Josh reads the refreshed packet and either approves apply / tweak / kill, then resume the weekly fresh high-fit scan. | **yes** to send |
