...
- **MOTION STATION: Kling Test A (solo idle) PASSED Josh's eye 2026-07-10** — first motion pass in project history. Official Kling API via `scripts/kling_generate.py` (key in .env, never committed). Test B (coffee sip — the Gate 2 killer gesture) queued in `exports/review/2026-07-09/motion_test_kit/RUN_TESTS.sh`; Test C (two-shot + screen text) after. If B+C pass → rebuild T+2 Ep.1 shot-by-shot: Blender keyframe stills → Kling motion → existing mux/caption pipeline.
