...
## Draft preflight snapshot (2026-08-05)
- Recheck status: both public/no-login gate endpoints returned 200 in a fresh local check.
- `nws_forecast` status: 200
- `openmeteo_models` status: 200
- What changed: the note now has a literal preflight snapshot to copy into the eventual run record instead of relying on memory.
