...
- **Portfolio reset 2026-07-25:** The Boring Report is now the flagship compounding data/product build (see `projects/portfolio-reset-2026-07-25.md`). **P2 Oracle Scorecard v0 evidence baseline built 2026-07-27 and internally reviewed 2026-07-30:** two protocol fact files, three SHA-256-verified source snapshots, deterministic scoring, rendered Markdown/JSON/HTML, and a fail-closed verifier live under `products/boring-report/scorecard/`. The acceptance note is `products/boring-report/scorecard/internal-review-2026-07-30.md`. It is deliberately architecture-evidence-only; the next increment is one traceable Aave V3 deployment feed map, followed by heartbeat observations and a sourced incident ledger before broader score claims. Draft support now exists in `products/boring-report/scorecard/output/aave-v3-deployment-feed-map-draft.md` with the official Aave address-book oracle anchor and the required fact-file shape, plus a reusable local run note at `products/boring-report/scorecard/output/aave-v3-deployment-feed-map-run-note.md`. The follow-up checklist now lives at `products/boring-report/scorecard/output/aave-v3-second-observation-checklist.md` so the next bundle can stay narrow and verifier-shaped. On 2026-08-14 the first bounded deployment row landed in `products/boring-report/scorecard/facts/aave-v3-deployment-feed-map.json` with a verified WETH source/feed/timestamp snapshot; its verifier now hashes all 91 bundle snapshots and reproduces the cited oracle/feed/decimals/latest-round claims. On 2026-08-15, `build_change_log.py` rendered that verified bundle into a local Markdown/JSON/HTML baseline pilot at `scorecard/output/aave-v3-change-log-pilot.*`; its test and live local HTTP check confirm the artifact is consumable, while correctly making no change, heartbeat, staleness, completeness, safety, or financial-advice claim. On 2026-08-16 the second independently verified WETH observation bundle landed in `products/boring-report/scorecard/facts/aave-v3-deployment-feed-map-2026-08-16.json`; feed and decimals stayed unchanged while the latest round fields advanced, so the next increment is a literal configuration-only comparison against the 2026-08-14 baseline. On 2026-08-17 the draft comparison pilot rendered that literal delta in Markdown/JSON/HTML and kept the heartbeat/staleness claim boundary at zero. W28 remains a useful internal reference, but no longer blocks the build behind repeated Gate Deck nagging.
