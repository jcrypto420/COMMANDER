...
export default function DocsIndex() {
  const groups = [...new Set(LIBRARY.map((doc) => doc.group))];
  return (
    <main className="shell" style={{ maxWidth: 760 }}>
      ...
      <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 0 }}>The documents that matter, one tap each. Always current — read straight from the repo.</p>
      {groups.map((group) => (
        <section key={group} style={{ marginTop: 18 }}>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {LIBRARY.filter((doc) => doc.group === group).map((doc) => (
              <a key={doc.slug} href={`/docs/${doc.slug}`} className="panel" style={{ display: 'block', padding: '14px 18px', borderRadius: 14, textDecoration: 'none', color: 'var(--text)' }}>
